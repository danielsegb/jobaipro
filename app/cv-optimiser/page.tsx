...
import { emptyCVData } from "@/lib/sampleData";
...
export default function CVOptimiserPage() {
  ...
  // Run CV Optimisation / Reformatting
        ...
        <div className="bg-white border-b border-slate-100 sticky top-16 z-30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center gap-6">
              <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">CV Optimizer</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {step === 1 && (
            <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
              <CVInputForm
                ...
                description="Upload your CV to structure, clean up, and professionally format it."
          ...
          {step === 2 && (
            <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
              {generationError && (
                <div className="mb-6">
                  <ErrorMessage message={generationError} onRetry={handleOptimiseCV} />
                </div>
              )}
              ...
              <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Provide job details to tailor your CV to this role, or skip to just reformat.
                  </p>
                </div>
                <button
                  onClick={handleOptimiseCV}
                  disabled={isGeneratingCV}
                  className="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-blue-600 hover:bg-slate-50 border border-slate-200 rounded-xl transition-all cursor-pointer"
                >
                  Skip & Reformat
                </button>
              </div>
              ...
              <JobDescriptionForm
                ...
                onBack={() => setStep(1)}
                onSubmit={handleOptimiseCV}
                isLoading={isGeneratingCV}
              />

              {isGeneratingCV && (
                <div className="mt-6">
                  <LoadingState type="cv" message={jobDescription.trim() ? "AI is tailoring your CV to the job description" : "AI is formatting and cleaning up your CV"} />
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4.5 bg-blue-50 border border-blue-100 rounded-2xl text-sm text-blue-900 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold">Need a tailored Cover Letter?</p>
                    <p className="text-xs text-blue-600/80 mt-0.5">Use your optimized CV and target job details to generate a customized cover letter.</p>
                  </div>
                </div>
                <Link href="/cover-letter" className="w-full sm:w-auto">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-slate-600 uppercase tracking-wider">Edit CV</h3>
                  <CVEditor
                    data={cvData}
                    onChange={setCvData}
